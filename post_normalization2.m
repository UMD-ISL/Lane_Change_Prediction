...
TotalLaneChange=51;%% Need to verify
NumberLCTest=3;
hiddenNodes=[15];
learnRates=[0.05];
...
load(strcat(rootIn,'feature_cal_6.mat'));
feature_cal_6=Feature;
load(strcat(rootIn,'feature_ECG_6.mat'));
feature_ECG_6=Feature;
load(strcat(rootIn,'feature_BELT_6.mat'));
feature_BELT_6=Feature;

load(strcat(rootIn,'feature_cal_7.mat'));
feature_cal_7=Feature;
load(strcat(rootIn,'feature_ECG_7.mat'));
feature_ECG_7=Feature;
load(strcat(rootIn,'feature_BELT_7.mat'));
feature_BELT_7=Feature;

load(strcat(rootIn,'feature_cal_8.mat'));
feature_cal_8=Feature;
load(strcat(rootIn,'feature_ECG_8.mat'));
feature_ECG_8=Feature;
load(strcat(rootIn,'feature_BELT_8.mat'));
feature_BELT_8=Feature;

%% Normalization all together
for i=1:NumberofSignalSelected
    featureAll_cal{i}=[feature_cal_1{i,1};feature_cal_2{i,1};feature_cal_3{i,1};feature_cal_4{i,1};feature_cal_5{i,1};feature_cal_6{i,1};...
        feature_cal_7{i,1};feature_cal_8{i,1}];
...
for j=1:NumberofSignalSelected
    ...
    video_cal_6{j}=[zeros(window_size_cal-1,Totalfeature+1);video((L_cal1+L_cal2+L_cal3+L_cal4+L_cal5+1:L_cal2+L_cal1+L_cal3+L_cal4+L_cal5+L_cal6),:)];
    video_cal_7{j}=[zeros(window_size_cal-1,Totalfeature+1);video((L_cal1+L_cal2+L_cal3+L_cal4+L_cal5+L_cal6+1:L_cal2+L_cal1+L_cal3+L_cal4+L_cal5+L_cal6+L_cal7),:)];
    video_cal_8{j}=[zeros(window_size_cal-1,Totalfeature+1);video((L_cal1+L_cal2+L_cal3+L_cal4+L_cal5+L_cal6+L_cal7+1:L_cal2+L_cal1+L_cal3+L_cal4+L_cal5+L_cal6+L_cal7+L_cal8),:)];
end
for i=1:NumberofTrips
...
for i=1:NumberofSignalSelected
    ...
    video_cal_6{i}=[video_cal_6{i},dataAll_cal_6(:,end)];
    video_cal_7{i}=[video_cal_7{i},dataAll_cal_7(:,end)];
    video_cal_8{i}=[video_cal_8{i},dataAll_cal_8(:,end)];
end
...
video_ECG_6=[zeros(window_size_ECG_raw-1,Totalfeature);feature_ECG_6];
video_ECG_7=[zeros(window_size_ECG_raw-1,Totalfeature);feature_ECG_7];
video_ECG_8=[zeros(window_size_ECG_raw-1,Totalfeature);feature_ECG_8];
...
video_BELT_6=[zeros(window_size_BELT_raw-1,Totalfeature);feature_BELT_6];
video_BELT_7=[zeros(window_size_BELT_raw-1,Totalfeature);feature_BELT_7];
video_BELT_8=[zeros(window_size_BELT_raw-1,Totalfeature);feature_BELT_8];
...
[~,maxIndex]=max([size(video_ECG_1,1);size(video_ECG_2,1);size(video_ECG_3,1);size(video_ECG_4,1);size(video_ECG_5,1);...
    size(video_ECG_6,1);size(video_ECG_7,1);size(video_ECG_8,1)]);
...
Index6=find(LookUpTable(:,1)==size(video_cal_6{1},1));
NEW_ECG6=video_ECG_6(LookUpTable(1:Index6,2),2:5);
NEW_ECG6=[zeros(window_size_cal-1,size(NEW_ECG6,2));NEW_ECG6];

Index7=find(LookUpTable(:,1)==size(video_cal_7{1},1));
NEW_ECG7=video_ECG_7(LookUpTable(1:Index7,2),2:5);
NEW_ECG7=[zeros(window_size_cal-1,size(NEW_ECG7,2));NEW_ECG7];

Index8=find(LookUpTable(:,1)==size(video_cal_8{1},1));
NEW_ECG8=video_ECG_8(LookUpTable(1:Index8,2),2:5);
NEW_ECG8=[zeros(window_size_cal-1,size(NEW_ECG8,2));NEW_ECG8];
...
video_cal_6=[video_cal_6,NEW_ECG6];
video_cal_7=[video_cal_7,NEW_ECG7];
video_cal_8=[video_cal_8,NEW_ECG8];
...
[~,maxIndex]=max([size(video_BELT_1,1);size(video_BELT_2,1);size(video_BELT_3,1);size(video_BELT_4,1);size(video_BELT_5,1);...
    size(video_BELT_6,1);size(video_BELT_7,1);size(video_BELT_8,1)]);
...
Index6=find(LookUpTable(:,1)==size(video_cal_6{1},1));
NEW_BELT6=video_BELT_6(LookUpTable(1:Index6,2),2:5);
NEW_BELT6=[zeros(window_size_cal-1,size(NEW_BELT6,2));NEW_BELT6];

Index7=find(LookUpTable(:,1)==size(video_cal_7{1},1));
NEW_BELT7=video_BELT_7(LookUpTable(1:Index7,2),2:5);
NEW_BELT7=[zeros(window_size_cal-1,size(NEW_BELT7,2));NEW_BELT7];

Index8=find(LookUpTable(:,1)==size(video_cal_8{1},1));
NEW_BELT8=video_BELT_8(LookUpTable(1:Index8,2),2:5);
NEW_BELT8=[zeros(window_size_cal-1,size(NEW_BELT8,2));NEW_BELT8];
...
video6=[video_cal_6,NEW_BELT6];
video7=[video_cal_7,NEW_BELT7];
video8=[video_cal_8,NEW_BELT8];
...
featureAll_ECG=[video1{:,8};video2{:,8};video3{:,8};video4{:,8};video5{:,8};video6{:,8};video7{:,8};video8{:,8}];
    for j=1:Totalfeature
       featureAll_ECG(:,j)=(featureAll_ECG(:,j)-min(featureAll_ECG(:,j)))./(max(featureAll_ECG(:,j))-min(featureAll_ECG(:,j)));
    end

 
featureAll_BELT=[video1{:,9};video2{:,9};video3{:,9};video4{:,9};video5{:,9};video6{:,9};video7{:,9};video8{:,9}];
for j=1:Totalfeature
       featureAll_BELT(:,j)=(featureAll_BELT(:,j)-min(featureAll_BELT(:,j)))./(max(featureAll_BELT(:,j))-min(featureAll_BELT(:,j)));
end
...
L6=size(video6{1,1},1);
L7=size(video7{1,1},1);
L8=size(video8{1,1},1);
video=[];
video=featureAll_ECG;
...
video_ECG_6=video((L1+L2+L3+L4+L5+1:L2+L1+L3+L4+L5+L6),:);
video_ECG_7=video((L1+L2+L3+L4+L5+L6+1:L2+L1+L3+L4+L5+L6+L7),:);
video_ECG_8=video((L1+L2+L3+L4+L5+L6+L7+1:L2+L1+L3+L4+L5+L6+L7+L8),:);
...
video6{1,8}=video_ECG_6;
video7{1,8}=video_ECG_7;
video8{1,8}=video_ECG_8;
...
video_BELT_6=video((L1+L2+L3+L4+L5+1:L2+L1+L3+L4+L5+L6),:);
video_BELT_7=video((L1+L2+L3+L4+L5+L6+1:L2+L1+L3+L4+L5+L6+L7),:);
video_BELT_8=video((L1+L2+L3+L4+L5+L6+L7+1:L2+L1+L3+L4+L5+L6+L7+L8),:);
...
video6{1,9}=video_BELT_6;
video7{1,9}=video_BELT_7;
video8{1,9}=video_BELT_8;
...
save(strcat(Outroot_origin,'video_final.mat'),'video1','video2','video3','video4','video5','video6','video7','video8');
...
save(strcat(rootIn,'video.mat'),'video1','video2','video3','video4','video5','video6','video7','video8');

Single_test3(rootIn,Outroot_origin,NumberofPoints_on_event,TotalLaneChange,hiddenNodes,learnRates,FeatureFilter,NumberLCTest)
