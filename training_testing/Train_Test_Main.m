...
for k = 1:10
    ...
    for mmm = 1:length(hidden_nodes)
        for nnn = 1:length(learning_rate)
            ...
            [netTrained, accuracy_train] = train_1NN(training_input, training_ground, testing_input, testing_ground, netConfig);

            disp('Finish training...'); toc;                                         % class 0 predict as class 1
            disp('Start testing...'  ); tic;

            [ true_positive, false_negative, false_positive, true_negative, accuracy_test ] = test_1NN(netTrained, testing_input, testing_ground);
