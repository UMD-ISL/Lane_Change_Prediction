    ...
    for k = 1:10
        ...
        for mmm = 1:length(hidden_nodes)
            for nnn = 1:length(learning_rate)
                ...
                [ netTrained, accuracy_train ] = train(training_input_20NN, training_ground_20NN, testing_input_20NN, testing_ground_20NN, netConfig, k);

                disp('Finish training...'); toc;                                         % class 0 predict as class 1
                disp('Start testing...'  ); tic;

                [ true_positive, false_negative, false_positive, true_negative, accuracy_test ] = test(netTrained, testing_input_20NN, testing_ground_20NN);
